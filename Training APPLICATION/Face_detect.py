...
from MongoDB_client import upload_file,upload_all_jpg_files
...
"""
def scan_people(frame,model,recog,name_list):
    gray = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    results = model(frame, stream=True)
    for r in results:
        boxes = r.boxes
        for box in boxes:
            ...
            if conf > 0.6:           
                if confs <= 80:                        
                    cvzone.putTextRect(frame, f'ID:{name_list[serial]}' , (max(0, x1), max(35, y1)), scale=0.6, thickness=1,offset=3)
                    cvzone.cornerRect(frame, (x1, y1, w, h), l=10)
                else:
                    cvzone.putTextRect(frame, "UNKNOWN" , (max(0, x1), max(35, y1)), scale=0.6, thickness=1,offset=3)
                    cvzone.cornerRect(frame, (x1, y1, w, h), l=10)
